		...
		for (int32 TokenIndex = 0; TokenIndex < TokenizedLine.Tokens.Num(); ++TokenIndex)
		{
			...
			if (!RunInfo.bIsWhitespace)
			{
				...
				else if (!TokenText.IsEmpty() && TChar<WIDECHAR>::IsAlpha(TokenText[0]))
				{
					...
					{
						FClingSemanticToken PreciseToken;
						const int32 OneBasedLine = LineIndex + 1;
						const int32 OneBasedColumn = ModelRange.BeginIndex + 1;
						if (SemanticInfoProviderToUse.TryGetSemanticTokenAtLocation(OneBasedLine, OneBasedColumn, PreciseToken)
							&& PreciseToken.Kind != EClingSemanticSymbolKind::Unknown)
						{
							const EClingSemanticSymbolKind PreciseKind = PreciseToken.Kind;
							if (PreciseKind == EClingSemanticSymbolKind::Namespace)
							{
								RunInfo.RunTypeName = TEXT("SyntaxHighlight.CPP.Namespace");
								RunInfo.Style = SyntaxTextStyle.NamespaceTextStyle;
							}
							else if (PreciseKind == EClingSemanticSymbolKind::Class)
							{
								RunInfo.RunTypeName = TEXT("SyntaxHighlight.CPP.Class");
								RunInfo.Style = SyntaxTextStyle.ClassTextStyle;
							}
							else if (PreciseKind == EClingSemanticSymbolKind::Struct || PreciseKind == EClingSemanticSymbolKind::Union)
							{
								RunInfo.RunTypeName = TEXT("SyntaxHighlight.CPP.Struct");
								RunInfo.Style = SyntaxTextStyle.StructTextStyle;
							}
							else if (PreciseKind == EClingSemanticSymbolKind::Enum)
							{
								RunInfo.RunTypeName = TEXT("SyntaxHighlight.CPP.Enum");
								RunInfo.Style = SyntaxTextStyle.EnumTextStyle;
							}
							else if (PreciseKind == EClingSemanticSymbolKind::TypeAlias)
							{
								RunInfo.RunTypeName = TEXT("SyntaxHighlight.CPP.Type");
								RunInfo.Style = SyntaxTextStyle.TypeTextStyle;
							}
							else if (PreciseKind == EClingSemanticSymbolKind::Function)
							{
								RunInfo.RunTypeName = TEXT("SyntaxHighlight.CPP.Function");
								RunInfo.Style = SyntaxTextStyle.FunctionTextStyle;
							}
							else if (PreciseKind == EClingSemanticSymbolKind::Template)
							{
								RunInfo.RunTypeName = TEXT("SyntaxHighlight.CPP.Template");
								RunInfo.Style = SyntaxTextStyle.TemplateTextStyle;
							}
							else if (PreciseKind == EClingSemanticSymbolKind::Variable)
							{
