...
// #endif
	...
	Interps.Add(StartInterpreterInternal(false));
...
cling::Interpreter* FClingRuntimeModule::StartInterpreterInternal(bool bBaseInterp)
{
	...
	if(bBaseInterp)
		return CreateInterp(Argv.Num(), Argv.GetData(), StringCast<ANSICHAR>(*LLVMDir).Get());
	if(BaseInterp)
		return CreateChildInterp(*BaseInterp,Argv.Num(), Argv.GetData(), StringCast<ANSICHAR>(*LLVMDir).Get());
	return nullptr;
}
